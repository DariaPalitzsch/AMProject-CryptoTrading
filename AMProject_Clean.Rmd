...
- Begründung des Schrittes von LM zu Lasso (Varianzreduktion, Korrelationseffecte, etc.)
...
df_daily <- df_daily %>%
  mutate(
    ...
    # signal = case_when(
    #   lead(log_return) > 0 ~ 1,   # Long
    #   lead(log_return) < 0 ~ -1,  # Short
    #   TRUE ~ 0               # No signal
    # )
  )
...
**Regress target return on 7-day momentum**
...
After performing the in-sample analysis, we now test our LASSO model Out-Of-Sample on the test data set: 
...
# TODO: Verlauf des MSE im Vergleich von IS zu OOS

#TODO: review code, does not work at the moment 

#ERICH: Comparison of compounded returns
```{r}
...
ggplot(df_test_extended, aes(x = target_return, y = pred_lasso)) +
...
ggplot(df_test_extended, aes(x = date)) +
