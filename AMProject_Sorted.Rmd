...
supervisor: "Prof. Dr. Andre Güttler, Tim Baumgartner"
date: "July 8, 2025"
output:
  pdf_document:
...
In this report, we design and evaluate a short-term trading strategy for Chainlink's native token LINK, a top-25 cryptocurrency that serves as a compensation token in a decentralized oracle network within the DeFi ecosystem \cite{CHL21}. LINK stands out as a compelling asset due to its high liquidity, large market cap, and fundamental utility, which make it less susceptible to manipulation and more likely to exhibit predictable price dynamics.
...
To prevent overfitting and perform automatic variable selection, we extend our linear modeling approach using the Least Absolute Shrinkage and Selection Operator (LASSO). The LASSO adds a penalty term to the standard OLS loss function, shrinking some coefficient estimates toward zero. This results in a sparse model that may improve predictive performance, particularly when dealing with multiple correlated predictors. Furthermore, reducing the number of relevant features allows for a better interpretation of simulation results \cite{ISL13}.
...
Additionally, we estimate the trading costs by applying a fixed fee per position change to simulate realistic trading costs. For the trading costs we estimate the average Ethereum gas price per day and multiply it by 60,000, which exceeds the factor for ERC-20 token transfer on the Ethereum blockchain but includes a safety margin \cite{GAS25}.
...
plot_2_inout <-  ggplot(df_test_extended, aes(x = date)) +
  ...
  labs(
    title = "Cumulative Returns: LASSO vs 7-Day Momentum vs LR",
    x = "Date", 
    y = "Cumulative Return",
    color = "Strategy"
    ) +
...
plot_3_inout <- ggplot(df_test_extended, aes(x = date)) +
  ...
  labs(
    title = "Cumulative Fees: LASSO vs 7-Day Momentum vs LR",
    x = "Date", 
    y = "Cumulative Fees (USD)",
    color = "Strategy"
    ) +
...
```{r lasso_cum_plot, fig.cap = "Cumulative Performance of Trading Strategies (Returns and Fees)", fig.align = "center", echo = FALSE, message = FALSE, warning = FALSE}
...
\bibitem{ISL13}
	\textsc{James}, G., \textsc{Witten}, D., \textsc{Hastie}, T., \textsc{Tibshirani}, R.:
	\textit{An Introduction to
...
\bibitem{RLR22}
  \textsc{Gunderson}, G.: 
  \textit{Returns and Log Returns} [online], 6 Feb. 2022, URL: gregorygundersen.com/blog/2022/02/06/log-returns [Retrieved: 08 Jul. 2025]
  
\bibitem{CHL21}
  \textsc{Breidenbach}, L., \textsc{Cachin}, C., \textsc{Chan}, B.: 
  \textit{Chainlink 2.0: Next Steps in the Evolution of
Decentralized Oracle Networks} [online], 15 Apr. 2021, URL: research.chain.link/whitepaper-v2.pdf [Retrieved: 08 Jul. 2025]

\bibitem{UOD21}
	\textsc{Zaremba}, A., \textsc{Bilgin}, M. H., \textsc{Long}, H., \textsc{Mercik}, A., \textsc{Szczygielski}, J. J.:
	\textit{Up or down? Short-term reversal, momentum, and liquidity effects in cryptocurrency markets},
...
\bibitem{GAS25}
  \textsc{MetaMask}: 
  \textit{User Guide: Gas} [online], URL: support.metamask.io/more-web3/learn/user-guide-gas [Retrieved: 08 Jul. 2025]


\bibitem{RSI25}
  \textsc{Wikipedia}: 
  \textit{Relative strength index} [online], URL: en.wikipedia.org/wiki/Relative\_strength\_index [Retrieved: 08 Jul. 2025]
  
\bibitem{ATR25}
  \textsc{Wikipedia}: 
  \textit{Average true range} [online], URL: en.wikipedia.org/wiki/Average\_true\_range [Retrieved: 08 Jul. 2025]
  
\bibitem{SMA25}
  \textsc{Wikipedia}: 
  \textit{Moving average} [online], URL: en.wikipedia.org/wiki/Moving\_average [Retrieved: 08 Jul. 2025]
  
\bibitem{MACD25}
  \textsc{Wikipedia}: 
  \textit{MACD} [online], URL: en.wikipedia.org/wiki/MACD [Retrieved: 08 Jul. 2025]
...
In our model \(n\) is set to 14. RSI is often associated with overbought and oversold conditions \cite{RSI25}.
...
From an economic perspective, the MACD value can reveal changes in the trend of a financial asset, since it captures differences between averages of long- and short-term periods. This makes it a strong predictor, especially for assets with infrequent trend changes \cite{MACD25}.
...
The SMA Difference indicates short-term momentum relative to the longer-term trend. A positive value suggests recent prices are rising faster than the broader trend, potentially signaling bullish sentiment, while a negative value may indicate emerging bearish pressure \cite{SMA25}.
...
The ATR reflects how much an asset typically moves in a day, with higher values indicating greater volatility. It does not indicate price direction but helps assessing risk and adjusting position sizes or stop-loss levels accordingly \cite{ATR25}.
