...
For our project we chose a dataset that provides price data of Chainlink's native LINK token at \textit{hourly frequency}. While such high-frequency data offers more granular insights, we chose to \textbf{aggregate the data to daily frequency} for the following reasons: 
...
To evaluate the temporal dependence structure of LINK’s daily log returns, we analyze the autocorrelation function (ACF) shown in Figure 2.
...
The ACF reveals no statistically significant linear dependence across the first 33 lags, suggesting that past returns do not linearly predict future returns—consistent with the weak-form Efficient Market Hypothesis (EMH). Nevertheless, this result does not preclude the existence of non-linear or directional patterns. Motivated by this, we proceed with a momentum-based trading strategy that exploits the sign of multi-day past returns to generate long or short signals.
...
Our results demonstrate that short-term predictability in the crypto space is possible using a structured, data-driven approach. We develop and backtest a daily trading strategy for LINK that issues “in” or “out” signals based on predicted next-day returns. The strategy enters the market only when the forecasted return exceeds the model-specific 75th percentile, thereby focusing on high-confidence signals. 
While a simple 7-day momentum signal proved insufficient, we enhanced performance by incorporating a broad feature set: momentum signals, volatility indicators, cross-asset spillovers, technical indicators, and calendar effects. Our final model uses Lasso regression to select only the most predictive variables and prevent overfitting. The Lasso model achieves the highest out-of-sample \( R^2 \) (33.6%) and directional accuracy (73.6%), outperforming both buy-and-hold and naïve momentum strategies even after accounting for trading costs. 
